...
class DocStripper {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      removeRepeatingHeadersFooters:
        options.removeRepeatingHeadersFooters !== false, // Default ON in Conservative mode
      mergeBrokenLines:
        options.mergeBrokenLines !== undefined
          ? options.mergeBrokenLines
          : false, // Default OFF in Conservative
      normalizeWhitespace:
  ...
  detectRepeatingHeadersFooters(text, pages) {
    ...
    for (let i = 0; i <= pages.length; i++) {
      ...
      for (let j = startIdx; j < endIdx; j++) {
        ...
        if (
          stripped &&
          !this.isHeaderFooter(stripped) &&
          !this.isPageNumber(stripped)
        ) {
  ...
  detectTableBlock(lines, startIdx) {
    ...
    if (startIdx >= lines.length - 2)
      return { isTable: false, endIdx: startIdx };

    const checkLines = lines.slice(
      startIdx,
      Math.min(startIdx + 10, lines.length),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (mergedLines.length > 0) {
        ...
        const nextIsList =
          i < lines.length - 1 &&
          lines[i + 1].trim() &&
          this.isListMarker(lines[i + 1]);
        const currentIsList =
          currentLine.trim() && this.isListMarker(currentLine);
        const prevIsHeader =
          this.isHeaderFooter(prevLine.trim()) ||
          this.isPageNumber(prevLine.trim());
        const currentIsHeader =
          this.isHeaderFooter(currentLine.trim()) ||
          this.isPageNumber(currentLine.trim());
  ...
  cleanText(text) {
    ...
    const repeatingHeadersFooters = this.detectRepeatingHeadersFooters(
      text,
      pageBoundaries,
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        !shouldRemove &&
        this.options.removeHeadersFooters &&
        this.isPageNumber(stripped)
      ) {
        headerFooterRemovedBeforeMerge++;
        shouldRemove = true;
      ...
      if (
        !shouldRemove &&
        this.options.removeRepeatingHeadersFooters &&
        repeatingHeadersFooters.has(stripped)
      ) {
        headerFooterRemovedBeforeMerge++;
        shouldRemove = true;
      }
    ...
    const whitespaceResult = this.normalizeWhitespace(
      text,
      this.options.keepTableSpacing,
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!stripped) {
        if (this.options.removeEmptyLines) {
          ...
          if (
            this.options.preserveParagraphSpacing &&
            prevNonEmptyLine !== null
          ) {
            // Check if next line is non-empty
            let nextNonEmptyIdx = i + 1;
            while (
              nextNonEmptyIdx < lines.length &&
              !lines[nextNonEmptyIdx].trim()
            ) {
              nextNonEmptyIdx++;
            }
      ...
      if (
        this.options.removePunctuationLines &&
        this.isPunctuationOnly(stripped)
      ) {
    ...
    stats.linesRemoved =
      lines.length - cleanedLines.length + headerFooterRemovedBeforeMerge;
    return {
  ...
  async extractTextFromPDF(file) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof pdfjsLib === "undefined") {
        reject(
          new Error("PDF.js library not loaded. Please refresh the page."),
        );
        return;
      }
...
class SmartCleaner {
  ...
  async ensureEngine() {
    ...
    try {
      ...
      if (!CreateEngine || typeof CreateEngine !== "function") {
        throw new Error(
          "WebLLM CreateMLCEngine not found. Please check WebLLM import.",
        );
      }
      ...
      try {
          ...
          progressCallback: (progress) => {
            ...
            if (
              ...
              const progressPercent = Math.min(
                (progress.loaded / progress.total) * 90 + 10,
                90,
              );
              this.updateProgress(
                progressPercent,
                `Loading model weights... ${Math.round(progressPercent)}%`,
              );
            } else if (typeof progress === "number") {
              // Some versions pass progress as a number 0-100
              this.updateProgress(
                progress,
                `Loading model weights... ${Math.round(progress)}%`,
              );
            } else {
        ...
        this.engine = await CreateEngine(this.fallbackModelId, {
          gpuMemoryUtilization: 0.7,
          progressCallback: (progress) => {
            if (
              progress &&
  ...
  buildPromptForChunk(chunk) {
    const linesWithIndices = chunk.lines
      .map((line, idx) => {
    ...
    if (this.settings.removeHeadersFooters) {
      removeInstructions.push(
        "- Page numbers: standalone digits (1, 2, 3), Roman numerals (I, II, III), single letters (A, B, C)",
      );
      removeInstructions.push(
        '- Headers/footers: "Page X of Y", "Confidential", "DRAFT", "Internal Use Only", "PROPRIETARY", etc.',
      );
    }

    if (this.settings.removePunctuationLines) {
      removeInstructions.push(
        "- Punctuation-only lines: lines with only symbols (---, ***, ===, ___, etc.)",
      );
    }

    if (this.settings.removeDuplicates) {
      removeInstructions.push(
        "- Consecutive duplicates: if line N is identical to line N-1, drop line N (keep first occurrence)",
      );
    }
    ...
    if (this.settings.removeEmptyLines) {
      if (this.settings.preserveParagraphSpacing) {
        removeInstructions.push(
          "- Extra empty lines: if there are 2+ consecutive empty lines, keep only one between paragraphs",
        );
      } else {
        removeInstructions.push(
          "- Empty lines: remove all empty/whitespace-only lines",
        );
      }
    }

    preserveInstructions.push(
      "- All content text (sentences, paragraphs, meaningful text)",
    );
    if (this.settings.preserveParagraphSpacing) {
      preserveInstructions.push(
        "- One empty line between paragraphs (for spacing)",
      );
    }
    ...
    if (temperament === "gentle") {
      modeGuidance =
        "GENTLE MODE: Be very cautious and preserve structure. When in doubt, keep content. Do not merge lines or normalize formatting aggressively.";
    } else if (temperament === "moderate") {
      modeGuidance =
        "MODERATE MODE: Balanced cleaning. You can merge broken lines when appropriate, but preserve formatting structure.";
    } else if (temperament === "thorough") {
      modeGuidance =
        "THOROUGH MODE: Comprehensive cleaning with normalization. Merge broken lines, normalize whitespace and Unicode, but preserve paragraph spacing for readability.";
    } else if (temperament === "aggressive") {
      modeGuidance =
        "AGGRESSIVE MODE: Maximum cleaning. Be thorough in removing noise, merge broken lines, normalize formatting, and remove all unnecessary spacing for compact output.";
    } else {
      // Fallback to old mode-based guidance
      modeGuidance =
        this.settings.cleaningModeType === "aggressive"
    ...
    if (this.settings.removeEmptyLines) {
      if (
        this.settings.preserveParagraphSpacing &&
        temperament !== "aggressive"
      ) {
        rules.push(
          "2. Empty lines: preserve ONE empty line between non-empty paragraphs, drop multiple empty lines",
        );
      } else {
        rules.push(
          "2. Empty lines: remove ALL empty lines (no paragraph spacing)",
        );
      }
    }
    if (this.settings.removeHeadersFooters) {
      rules.push(
        '3. Page numbers and headers/footers: drop ONLY if the entire line matches known patterns (e.g., "1", "III", "A", "Page X of Y", "Confidential")',
      );
    }
...
Respond with JSON only:`;
  ...
  async cleanText(text, settings = null) {
    ...
    try {
      ...
      const maxChunkSize =
        textLength > 50000 ? 4000 : textLength > 20000 ? 3500 : 3000;
      ...
      for (
        ...
        const batchPromises = batch.map(async (chunk, batchIdx) => {
          ...
          this.updateProgress(
            progressPercent,
            `Processing chunk ${chunkIdx + 1}/${totalChunks}...`,
          );

          try {
            const { systemPrompt, userPrompt } =
              this.buildPromptForChunk(chunk);
            ...
            if (!plan) {
              console.warn(
                `Failed to parse LLM response for chunk ${chunkIdx + 1}, using original chunk`,
              );
      ...
      if (this.settings.normalizeWhitespace) {
        const whitespaceResult = this.normalizeWhitespace(
          cleanedText,
          this.settings.keepTableSpacing,
        );
...
class App {
  ...
  async initializeWorker() {
    ...
    try {
      ...
      console.warn(
        "Failed to initialize WebWorker, falling back to main thread:",
        error,
      );
      return null;
    }
  ...
  initializeElements() {
    ...
    this.temperamentDescription = document.getElementById(
    ...
    this.removePunctuationLines = document.getElementById(
      "removePunctuationLines",
    );
    this.dehyphenate = document.getElementById("dehyphenate");
    this.preserveParagraphSpacing = document.getElementById(
  ...
  loadSettings() {
    try {
      ...
      if (saved) {
        ...
        // Restore auto-start
        if (
          settings.autoStartAfterUpload !== undefined &&
          this.autoStartAfterUpload
        ) {
        ...
        if (
          settings.removeHeadersFooters !== undefined &&
          this.removeHeadersFooters
        ) {
        ...
        if (
          settings.removePageNumbers !== undefined &&
          settings.removePageNumbers === false
        ) {
        ...
        if (
          settings.removePunctuationLines !== undefined &&
          this.removePunctuationLines
        ) {
          this.removePunctuationLines.checked = settings.removePunctuationLines;
        }
        if (
          settings.preserveParagraphSpacing !== undefined &&
          this.preserveParagraphSpacing
        ) {
          this.preserveParagraphSpacing.checked =
            settings.preserveParagraphSpacing;
        }
        ...
        if (
          settings.normalizeWhitespace !== undefined &&
        ...
        const hasShownRestoreNotification = sessionStorage.getItem(
          "docstripper_restore_shown",
        );
  ...
  saveSettings() {
    try {
      const settings = {
        cleaningTemperament: this.cleaningTemperament
          ? parseInt(this.cleaningTemperament.value)
          : 0,
        ...
        removeRepeatingHeadersFooters:
          this.removeHeadersFooters?.checked ?? true, // Automatically enabled when removeHeadersFooters is enabled
        removeDuplicates: this.removeDuplicates?.checked ?? true,
        removePunctuationLines: this.removePunctuationLines?.checked ?? true,
        preserveParagraphSpacing:
          this.preserveParagraphSpacing?.checked ?? true,
  ...
  updateTemperamentFromValue(value) {
    ...
    this.cleaningModeType =
      mode === "aggressive" || mode === "thorough"
        ? "aggressive"
        : "conservative";
  ...
  applyTemperamentDefaults(mode) {
    ...
    switch (mode) {
      ...
      case "aggressive":
        ...
        if (this.normalizeUnicode) this.normalizeUnicode.checked = true;
        if (this.preserveParagraphSpacing)
          this.preserveParagraphSpacing.checked = false;
  ...
  applyModeDefaults() {
    if (this.cleaningModeType === "aggressive") {
      ...
      if (this.removePunctuationLines)
        this.removePunctuationLines.checked = true;
      if (this.preserveParagraphSpacing)
        this.preserveParagraphSpacing.checked = true;
      if (this.dehyphenate) this.dehyphenate.checked = true;
  ...
  handleFiles(files) {
    ...
    if (
      this.autoStartAfterUpload &&
      this.autoStartAfterUpload.checked &&
      this.startBtn
    ) {
      setTimeout(() => {
        if (this.files.length > 0) {
  ...
  async processFiles() {
    ...
    const settings = {
      removeEmptyLines: this.removeEmptyLines
        ? this.removeEmptyLines.checked
        : true,
      removeHeadersFooters: this.removeHeadersFooters
        ? this.removeHeadersFooters.checked
        : true, // Includes page numbers removal
      removeRepeatingHeadersFooters: this.removeHeadersFooters
        ? this.removeHeadersFooters.checked
        : true, // Automatically enabled when removeHeadersFooters is enabled
      removeDuplicates: this.removeDuplicates
        ? this.removeDuplicates.checked
        : true,
      removePunctuationLines: this.removePunctuationLines
        ? this.removePunctuationLines.checked
        : true,
      preserveParagraphSpacing: this.preserveParagraphSpacing
        ? this.preserveParagraphSpacing.checked
        : true,
      dehyphenate: this.dehyphenate ? this.dehyphenate.checked : true,
      mergeBrokenLines: this.mergeBrokenLines
        ? this.mergeBrokenLines.checked
        : false,
      normalizeWhitespace: this.normalizeWhitespace
        ? this.normalizeWhitespace.checked
        : false,
      normalizeUnicode: this.normalizeUnicode
        ? this.normalizeUnicode.checked
        : false,
      keepTableSpacing: this.keepTableSpacing
        ? this.keepTableSpacing.checked
        : true,
      ...
      for (const file of this.files) {
        ...
        if (useSmartMode) {
          try {
            ...
            const cleanedResult = await this.smartCleaner.cleanText(
              text,
              settings,
            );
            ...
            console.warn(
              "Smart clean failed, falling back to Fast clean:",
              smartError,
            );
            // Fallback to Fast Clean
            this.showToast(
              "Smart Clean unavailable, using Fast Clean",
              "error",
            );
          ...
          const text = await this.stripper.readTextFile(file);
          const useWorker =
            text.length > 100 * 1024 || text.split("\n").length > 5000;

          if (useWorker) {
            try {
              const cleanedResult = await this.cleanTextInWorker(
                text,
                settings,
              );
              result = {
        ...
        if (result.success) {
          ...
          totalStats.duplicatesCollapsed +=
            result.stats.duplicatesCollapsed || 0;
          totalStats.emptyLinesRemoved += result.stats.emptyLinesRemoved || 0;
          totalStats.headerFooterRemoved +=
            result.stats.headerFooterRemoved || 0;
          totalStats.punctuationLinesRemoved +=
            result.stats.punctuationLinesRemoved || 0;
          totalStats.dehyphenatedTokens =
            (totalStats.dehyphenatedTokens || 0) +
            (result.stats.dehyphenatedTokens || 0);
          totalStats.repeatingHeadersFootersRemoved =
            (totalStats.repeatingHeadersFootersRemoved || 0) +
            (result.stats.repeatingHeadersFootersRemoved || 0);
          totalStats.mergedLines =
            (totalStats.mergedLines || 0) + (result.stats.mergedLines || 0);
        }
      }
    } finally {
    ...
    setTimeout(() => {
      ...
      if (firstDownloadBtn) {
        firstDownloadBtn.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
  ...
  formatStatsLine(stats, originalLineCount) {
    ...
    const totalHeadersFooters =
      (stats.headerFooterRemoved || 0) +
      (stats.repeatingHeadersFootersRemoved || 0);
    const parts = [
  ...
  displayResults(results, totalStats) {
    ...
    // Brief stats line
    const briefStats = [];
    if (totalStats.mergedLines > 0)
      briefStats.push(`Merged ${totalStats.mergedLines} lines`);
    if (totalStats.dehyphenatedTokens > 0)
      briefStats.push(`Dehyphenated ${totalStats.dehyphenatedTokens} tokens`);
    if (totalStats.repeatingHeadersFootersRemoved > 0)
      briefStats.push(
        `Removed ${totalStats.repeatingHeadersFootersRemoved} repeating headers/footers`,
      );
    ...
    results.forEach((result, index) => {
      ...
      const modeBadge =
        result.mode === "smart"
          ? '<span class="mode-badge smart">🤖 Smart Clean</span>'
          : result.mode === "fast-fallback"
            ? '<span class="mode-badge fallback">⚡ Fast (Fallback)</span>'
    ...
    const statsSummaryEl =
      this.resultsContainer.querySelector(".stats-summary");
    if (statsSummaryEl) {
      ...
      !bulkContainer.parentElement &&
      this.resultsContainer.parentElement
    ) {
      // Fallback: append at top of results section
      this.resultsContainer.parentElement.insertBefore(
        bulkContainer,
      ...
      btn.addEventListener("click", (e) => {
        const index = parseInt(
          e.target.dataset.index ||
            e.target.closest(".download-btn").dataset.index,
        );
        if (results[index]) {
    ...
    document.querySelectorAll(".copy-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        const index = parseInt(
          e.target.dataset.index || e.target.closest(".copy-btn").dataset.index,
        );
        if (results[index]) {
  ...
  downloadFile(result) {
    try {
      const blob = new Blob([result.cleanedText], {
  ...
  async downloadAll(results) {
    ...
    try {
      ...
      saveAs(blob, name);
      this.showToast(
        `✅ ${results.length} files cleaned and downloaded`,
        "success",
        4000,
      );
    } catch (e) {
...
function customizeGoogleTranslate() {
  ...
  const checkTranslate = setInterval(() => {
    ...
    if (translateElement) {
      ...
      const link =
        translateElement.querySelector("a.VIpgJd-ZVi9od-xl07Ob-lTBxed") ||
...
document.addEventListener("keydown", (e) => {
  const isModifierPressed = e.ctrlKey || e.metaKey; // e.metaKey is cmd (on mac at least)
  const key = e.key.toLowerCase(); // converting the key to lowercase

  // Only work if start button is visible and enabled
  switch (key) {
    case "u": // Upload file
      e.preventDefault();
      this.fileInput.click(); // Simulate button click
      break;
    case "enter":
      e.preventDefault();
      this.startBtn.click();
